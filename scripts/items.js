...
return <div class="card"> <div style="font-weight:700;font-size:18px">${esc(item.itemName)}</div> <div style="color:#555">Type: ${esc(label)}</div> <div style="margin-top:8px">Location: <strong>${esc(item.locationFound||item.lostLocation||'-')}</strong></div> <div>Date: <strong>${esc(item.dateFound||item.lostDate||'-')}</strong></div> <div style="margin-top:8px">Description: ${esc(item.description||'')}</div> ${item.imageUrl?`:''}

Tracking ID: ${esc(item.trackingId)}
